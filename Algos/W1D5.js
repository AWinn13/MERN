...
function quickSort(nums = [], left = 0, right = nums.length - 1) {
    // base case makes sure that our pivots never cross || base case occurs when the array is the lengh of 0 or 1
    if (left >= right){
        return nums
    }
    // able to use partition twice by setting it to a variable
    let partitionIdx = partition(nums, left, right);
    // partition the left side of the array 
    quickSort(nums, left, partitionIdx - 1);
    // partition the right side of the array
